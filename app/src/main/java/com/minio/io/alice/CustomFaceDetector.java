...
public class CustomFaceDetector extends Detector<Face> {
   ...
   /* public CustomFaceDetector(Context context, Detector<Face> delegate) {
    ...
    public SparseArray<Face> detect(Frame frame) {
        ...
        try{
            ...
            if (faces.size() > 0) {
                ...
                mainActivity.getPreviewSize().increaseZoom(calculateOptimalZoomFactor(frame.getMetadata(),
                        faces.clone()));
    ...
    private int calculateOptimalZoomFactor(Frame.Metadata metaData, SparseArray<Face> faces) {
        Rect boundingBox = new Rect(0,0,metaData.getWidth(),metaData.getHeight());
        Rect combinedRectangle = new Rect();
        final int zoomOutBorderSize = 50;
        final int  nozoomBorderSize = 75;
        final int  zoomBoost = 5;
